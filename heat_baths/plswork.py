...
#hot bath at left end only, cold bath at right end only
...
def chain(NSpins, total_sweeps, burn_in, T_left, T_right, J, mu, H):
    ...
    for sweep in range(total_sweeps):
        for _ in range(NSpins):            
            ...
            #if spin interacting with spin at left end, use cold bath temperature
            if r==0:
                beta=beta_h
            elif r==NSpins-1:
                beta=beta_c
            else:
                left_aligned=(spin[r-1]==spin[r])
                right_aligned=(spin[r+1]==spin[r])
...
def sim():
    ...
    plt.tight_layout()
    plt.savefig("open_heat_flux_and_profile.png",dpi=300)
    df = pd.DataFrame(results_list)
    df.to_csv("open_heat_transport.csv", index=False)
    plt.show()
