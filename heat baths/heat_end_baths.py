...
def chain(NSpins, total_sweeps, burn_in, T_left, T_right, J, mu, H):
    ...
    betas=1.0/np.linspace(T_left,T_right,NSpins)
...
def sim():
    ...
    plt.tight_layout()
    plt.savefig("endslinear_heat_flux_and_profile.png",dpi=300)
    df = pd.DataFrame(results_list)
    df.to_csv("endslinear_heat_transport.csv", index=False)
    plt.show()
